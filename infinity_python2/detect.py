...
def detect_and_cut_rectangles(image_path, output_directory):
    ...
    divLst.append(width1*0.4)
    divLst.append(height1*0.5)
    divLst2.append(width1*0.6)
    divLst2.append(height1*0.5)
...
# Cut and save each detected rectangle as a separate image
    ...
    for i, contour in enumerate(contours):
      ...
      if len(approx) == 4:
          ...
          # Save the cropped image as a separate file to new folder
          output_pathx = os.path.join("ext", f"rectangle_{i + 1}.png")
          cv2.imwrite(output_pathx, cropped_image)
          print(f"Saved rectangle {i + 1} as {output_pathx}")
          rectangle_ext_info = {
              'x': x,
              'y': y,
              'width': width,
          ...
          rectangles_ext.append(rectangle_ext_info)
        ...
        #   checking div conditions
          for div in enumerate(rectangles_ext) :
              if div[1]["x"] < divLst[0] and div[1]["y"] < divLst[1] :
                  print("yess left")
              elif div[1]["x"] > divLst[0] and div[1]["y"] < divLst[1]:
                  print("yess right")
              else :
                  print("no")
...
for i, rectangle in enumerate(rectangles):
    ...
    rectangles.pop(1)
    # if i == 0 :
    #     response = requests.get(f"https://api.ocr.space/parse/imageurl?apikey=K88933154488957&url=https://262a-103-103-215-118.ngrok-free.app/cut_image/rectangle_{i+1}_{i+1}_{i+1}.png")
    # elif i == 1 :
    #     response = requests.get(f"https://api.ocr.space/parse/imageurl?apikey=K88933154488957&url=https://262a-103-103-215-118.ngrok-free.app/cut_image/rectangle_{i}_{i}_{i + 1}.png")
    # elif i== 2 :
    #     response = requests.get(f"https://api.ocr.space/parse/imageurl?apikey=K88933154488957&url=https://262a-103-103-215-118.ngrok-free.app/cut_image/rectangle_{i}_{i + 1}_{i-1}.png")
    # elif i == 3 :
    #     response = requests.get(f"https://api.ocr.space/parse/imageurl?apikey=K88933154488957&url=https://262a-103-103-215-118.ngrok-free.app/cut_image/rectangle_{i-1}_{i}_{i-1}.png")
    response = 409
    if response == 200:
