...
def detect_and_cut_rectangles(image_path, output_directory):
    ...
    for i, contour in enumerate(contours):
        ...
        if len(approx) == 4:
            ...
            for rect in rectangles:
                if rect[0] < x and rect[1] < y and rect[0] + rect[2] > x + width and rect[1] + rect[3] > y + height:
                    is_nested = True
                    break
            if is_nested:
                nested_rectangles.append((x, y, width, height))
            else:
    ...
    # Process nested rectangles
    for i, nested_rect in enumerate(nested_rectangles):
        x, y, width, height = nested_rect
        # Crop the nested rectangle from the original image
        cropped_image = image[y:y + height, x:x + width]

        # Save the cropped image as a separate file
        output_path = os.path.join(output_directory, f"nested_rectangle_{i + 1}.png")
        cv2.imwrite(output_path, cropped_image)
        print(f"Saved nested rectangle {i + 1} as {output_path}")
...
#     response = requests.get(f"https://api.ocr.space/parse/imageurl?apikey=K82315508888957&url=https://abbd-103-103-214-251.ngrok-free.app/cut_image/rectangle_{i+1}.png")
...
#                 # else :
#                 div_str += "<img src='./a.jpg' style='height : 200px'; width : 'auto' />"
#                 # imageUl += 1
#         elif text == "anchor":
...
# </div>
# <div class="col-md-6">
# <img src="./a.jpg" class="img-thumbnail" alt="image" />
# </div>
# </div>
#             <!--bootstrap js-->
